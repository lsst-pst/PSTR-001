...
\end{minipage}
\\ \hline
\href{https://jira.lsstcorp.org/secure/Tests.jspa#/testCase/LVV-T2841}{LVV-T2841}
& {\small Lynne Jones } & {\small  } &
\begin{minipage}[]{6cm}
\smallskip
{\small  }
...
\end{minipage}
\\ \hline
\href{https://jira.lsstcorp.org/secure/Tests.jspa#/testCase/LVV-T2843}{LVV-T2843}
& {\small Lynne Jones } & {\small  } &
\begin{minipage}[]{6cm}
\smallskip
{\small  }
...
fO\_Nv.\\
Alternatively, the area on sky are which the minimum number of visits
per pointing is Nv1Sum can also be calculated, and is reported via MAF
as fO\_Area.\\[2\baselineskip]
...
\end{tabular}
 Description \\
{\footnotesize
Run the Jupyter notebook at
\textless{}lsst-pst/pstr-001/..\textgreater{}
...
\end{tabular}
 Description \\
{\footnotesize
Confirm outputs match expected values, including values for fO\_Nv and
fO\_area\\[2\baselineskip]
...
manner.\\
The original statement of ``near uniformity'' over this time span does
not account for the peak in this timescale caused by standard pairs of
...
written to account for the intent of the requirement -- that there are a
significant fraction of visits in the timespan 40s - 20 minutes, as well
as visits between 20 - 30 minutes.
...
\end{tabular}
 Description \\
{\footnotesize
Run notebook at \textless{}lsst-pst/pstr-001\textgreater{}
...
will be at least SIGpm = 1.0 mas for sources r\textgreater{}24. }\mbox{}\\
...
be at least SIGpm = 1.0 mas for sources r\textgreater{}24.\\
...
by survey simulations.\\[2\baselineskip]Survey simulations use a model
of the telescope to estimate slew times (including filter change times),
coupled with scheduler choices for each successive visit. The times
between successive visits can be evaluated from these simulations.
...
\end{tabular}
 Description \\
{\footnotesize
Confirm average time between visits matches
expectations\\[2\baselineskip]
...
of the telescope to estimate slew times (including filter change times),
coupled with scheduler choices for each successive visit. The times
between successive visits can be evaluated from these
simulations.\\[2\baselineskip]
...
\end{tabular}
 Description \\
{\footnotesize
Run Jupyter notebook at PSTR-001
...
\end{tabular}
 Description \\
{\footnotesize
Confirm simulated median time between visits matches expectations
...
Verify the scheduler responds to previous survey
performance.\\[2\baselineskip]Previous survey performance here is
interpreted as variations in previous sky coverage, for example
responding after a month-long shutdown where no visits were acquired.\\
...
longer timescales such as after one year or end of
survey.~\\[2\baselineskip]
...
\end{tabular}
 Description \\
{\footnotesize
Confirmed expected results.\\[2\baselineskip]
...
\paragraph{ LVV-T2841 - Verify the scheduler can respond to nightly technical performance. }\mbox{}\\

Version \textbf{1}.
Status \textbf{Draft}.
Open  \href{https://jira.lsstcorp.org/secure/Tests.jspa#/testCase/LVV-T2841}{\textit{ LVV-T2841 } }
test case in Jira.

Verify that the scheduler responds to nightly technical performance.\\
Nightly technical performance here is taken to mean that the scheduler
can respond to variations in the observatory hardware capabilities, such
as a temporary limitation on the dome or telescope movement capabilities
or increased readout time.\\
The timescale for response include short term appropriate responses
(such as changing the requested observations to account for a changed
slew time), and also that the scheduler adjusts for these responses over
longer timescales in a way that still achieves survey goals as best as
possible (obviously longer slew times continuing for a 10 years will
result in fewer visits overall, thus compromised survey goals).
...
\end{tabular}
 Description \\
{\footnotesize
Run Jupyter notebook \textless{} xxx\textgreater{}~
...
\paragraph{ LVV-T2843 - Verify the scheduler can respond to internal and/or external constraints }\mbox{}\\

Version \textbf{1}.
Status \textbf{Draft}.
Open  \href{https://jira.lsstcorp.org/secure/Tests.jspa#/testCase/LVV-T2843}{\textit{ LVV-T2843 } }
test case in Jira.

Verify the scheduler responds to internal and/or external
constraints.\\[2\baselineskip]Internal and/or external constraints is
wide ranging and overlaps with nightly technical performance and
observing conditions.\\
Here ``internal and/or external constraints'' will be interpreted as
meaning that the scheduler can respond to a change in survey strategy
goals (the change in goals being an external constraint that was
changed).\\
This would be a long-term response, so we will look at the impact on
observations after a year and at end of survey.~
